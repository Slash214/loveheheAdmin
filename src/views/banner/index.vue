<template>
  <el-button class="mb20" type="primary" @click="addImage">增加轮播图</el-button>
  <div class="banner">
  ...
  <modal :title="'更新轮播图'" :isVisible="isVisiable" :width="700" @cancel="onCancel">
    <div class="box">
      <el-input placeholder="图片id" v-model="params.id" v-if="!stae"></el-input>
      ...
      <el-button @click="addBanner">{{ stae ? '增加' : '更新' }}</el-button>
    </div>
  </modal>
</template>
...
let stae = ref<boolean>(true) // true 表示增加  false 表示更新
...
const addBanner = async () => {
  if (stae.value) {
    if (!params.link || !params.picture) {
      ElMessage.warning('参数不能为空')
      return
    }

    const { data, message, code } = await createBanner({
      link: +params.link,
